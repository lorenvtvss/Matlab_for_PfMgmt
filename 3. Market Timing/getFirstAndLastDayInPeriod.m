...
% Dates should be provided as numeric in the format:
% YYYYMMDD, YYMMDD, or MMDD. 
%
% The granularity of the period is defined by the number of digits  
% that are removed from the date list. 
% 2 digits:  will convert daily data to monthly data 
%            or monthly data to annual data. 
% 4 digits:  will convert daily data to annual data.
...
isNewPeriod = diff(trimmedDate);
% Same as: isNewPeriod = trimmedDate(2 : nObs, 1) - trimmedDate(1 : nObs - 1, 1);

% Generate a list with the last days, and use it to generate the list of
% the first days
lastDayList = find(isNewPeriod);
