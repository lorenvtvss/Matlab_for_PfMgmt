% Solution to the market timing example
clc
clear
close all
...
% Load the data
data = xlsread('FFDaily.xls','Sheet1','A2:E25085');
dates = data(:, 1);
nDays = length(dates)
...
% Plot equity lines using daily returns, both for total and excess returns
...
for m = 1 : nMonths
    ...
    last = lastDayList(m);
    monthlyVar(m, :) = var(xsReturns(max(first - lag, 1) : last - lag, :));    
end
...
if (monthlyDataFromFile)
    monthlyData = xlsread('FFMonthly.xls', 'Sheet1', 'A2:E1144');
